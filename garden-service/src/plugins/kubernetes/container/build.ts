...
const buildStatusHandlers: {
  [mode in ContainerBuildMode]: BuildStatusHandler
} = {
...
const remoteBuild: BuildHandler = async (params) => {
  ...
  await pRetry(() => execa("rsync", ["-vrpztgo", "--relative", "--delete", src, destination]), {
    retries: 3,
    minTimeout: 500,
  })
  ...
  if (provider.config.buildMode === "cluster-docker") {
    ...
    const buildRes = await execInBuilder({
      provider,
      log,
      args,
    ...
    const pushRes = await execInBuilder({
      provider,
      log,
      args: pushArgs,
      timeout: 300,
